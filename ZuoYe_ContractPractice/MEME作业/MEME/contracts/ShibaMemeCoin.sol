...
contract ShibaMemeCoin is ERC20, Ownable, ReentrancyGuard {
    ...
    */    
    ...
    function _applyTradingLimits(address from, address to, uint256 amount) private {
        ...
        // 交易时间间隔限制
        /*
            这段代码 ammPairs[to] 为真时才执行验证，意味着该限制专门针对“卖出”行为。
            1. 为什么只限制 ammPairs[to]（卖出）？
                在 $SMEME$ 这类项目中，设置 minTimeBetweenTx（两次交易最小间隔）的主要目的是防止抛售（Anti-Dump）和对抗夹子机器人（Anti-MEV/Bot）。
                针对卖出： 当 to 是 ammPairs（即流动性池地址）时，说明用户正在将代币换成 ETH。如果一个地址在极短时间内连续卖出，会对币价造成巨大的下行压力。
                放行转账： 普通的钱包间转账（Wallet to Wallet）不涉及流动性池，不会直接影响币价。
                如果不加 ammPairs[to]，用户想把币分发到自己的两个小号钱包，也得等上几十秒，这会极大伤害用户体验。

            2. 交易类型的逻辑判断
            合约通过 from 和 to 是否为 ammPair 来识别交易性质：
            场景,条件判断,性质,是否触发该 if 逻辑
            买入,ammPairs[from] == true,用户从池子拿币,不触发（除非代码里也检查了 from）
            卖出,ammPairs[to] == true,用户把币扔回池子,✅ 触发（限制抛售频率）
            普通转账,均为 false,钱包对钱包,不触发（为了操作灵活）
            3. 为什么“转账”通常不限制时间间隔？
                非抛售行为： 仅仅是代币位置的移动，不产生卖盘压力。
                合规性与流动性： 很多代币需要被转移到交易所、质押合约或朋友手中。如果每一笔转账都加时间锁，会显得这个代币极其“中心化”且难以使用。
                Gas 效率： 每次转账都去读写 lastTradeTime（存储变量）是非常耗 Gas 的。只在关键的“卖出”环节进行验证，可以为普通用户节省开支。
            4. 潜在的漏洞：绕过限制？
                如果只限制 ammPairs[to]，确实存在一种绕过方法：
                手法： 机器人可以先将代币从“钱包 A”转账给“钱包 B”（无时间限制），然后立即从“钱包 B”卖给池子。
                防御： 专业的合约通常还会配合 maxTransactionAmount（单笔最大交易额） 或 maxWalletAmount（单钱包最大持仓） 来多维度封堵。    

            有些严格的合约会写成： if (ammPairs[from] || ammPairs[to])

            这样买入也会有冷却时间。如果你现在的代码只写了 ammPairs[to]，说明项目方的策略是：“欢迎随时进来（买入），但你想走（卖出）的时候得按规矩排队。” 
            这是一种典型的通过限制流动性出口来维持币价稳定的做法。

            为什么 ammPairs[from] == true 是买入？ -- 池子给用户
            池子（钱柜）把代币给了你，意味着你在 DEX 界面上点击了“Swap”，用你的 ETH 换走了池子里的代币。 
            动作结果： 你的钱包里代币变多了。所以，这在代币合约的视角下就是一次 “买入” 行为。    
            简单来说：用户把 ETH 丢进了池子	买入 (Buy)

            为什么 ammPairs[to] == true 是卖出？ -- 用户给池子
            你把手中的代币发送给了池子（钱柜），池子收到代币后，会根据算法把等值的 ETH/USDT 打回给你。 
            动作结果： 代币回到了池子里，你拿走了钱。所以，这在合约视角下就是一次 “卖出” 行为
            简单来说：用户从池子里拿走了 ETH  卖出 (Sell)

            用户给用户from: User -> to: User
            不涉及池子，没有 ETH 交换	转账 (Transfer)
